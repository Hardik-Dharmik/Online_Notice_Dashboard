...
def register(request):
    if request.method=='POST':
        user_image=request.POST.get("fileupload")
        ...
        if len(rollno)<4:
            messages.warning(request,"Roll no Should be 4 digit")
        ...
        newprofile=Profile(image=user_image,user=new_user,Date_Of_Birth=dob,phone=phno,Roll_No=rollno,Department=dept,Year_Of_Study=yos,Gender=gender)
        newprofile.save()
